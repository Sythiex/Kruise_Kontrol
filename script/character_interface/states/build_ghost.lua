...
function Build_ghost:update()
    ...
    local entities = self.target.surface.find_entities_filtered { type = "tree", position = self.target.position, radius = self.target.get_radius(), to_be_deconstructed = false }
    if next(entities) ~= nil then
